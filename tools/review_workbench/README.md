...
演示使用隔离的临时文件和模拟 IDA，不操作真实项目。当前 18 项离线测试通过，覆盖文件写回、撤销、冲突检查、地址映射及模拟 IDA RPC。尚未完成真实 Linux IDA 和 Qt 桌面环境联调。

开发界面时，在 frontend 下执行 npm ci 和 npm run build。发布时需保留重新生成的 web 目录。

## 界面与性能更新

本次更新显式设置按钮和下拉选项的深色背景，主文字与代码字号为 14px、辅助文字为 12px。长函数列表只渲染可见区域附近的行；代码高亮按内容缓存；源码索引按文件读取函数名，读取过程不占用操作锁；后台状态刷新不再重叠请求，空闲刷新间隔为 10 秒。

更新已有安装时，覆盖整个 web 目录以及 model.py 后重启。无需重新加载 IDA 中的 bridge.py。也可以直接覆盖整个交付目录。
